...
function EggQualityDetection() {
  ...
  useEffect(() => {
    const Ref = ref(db, "egg-quality-data");
  ...
  useEffect(() => {
    const topRef = ref(db, "egg-quality-data-top");
  ...
  useEffect(() => {
    const bottomRef = ref(db, "egg-quality-data-bottom");
                 ...
                 <p className="text-5xl font-bold text-green-700">
                   <p className="text-5xl font-bold text-green-700">
                     {latestPrediction.Result
                       ? latestPrediction.Result.charAt(0).toUpperCase() +
                         latestPrediction.Result.slice(1).toLowerCase()
                       : "Unknown"}
                   </p>
                 </p>
               </span>
       ...
       <div className="grid grid-cols-1 gap-6 px-6 mt-8 text-2xl sm:grid-cols-2 md:grid-cols-4 md:px-24">
         {images.slice(-2).map((img, index) => (
           <div key={`top-${index}`} className="flex flex-col items-center">
             ...
             <p className="mt-1 font-semibold text-left text-black">
               {topData[topData.length - 2 + index]?.Result
                 ? topData[topData.length - 2 + index]?.Result.charAt(
                     0
                   ).toUpperCase() +
                   topData[topData.length - 2 + index]?.Result.slice(
                     1
                   ).toLowerCase()
                 : "Unknown"}
             </p>
         ...
         {images.slice(-2).map((img, index) => (
           <div key={`bottom-${index}`} className="flex flex-col items-center">
             ...
             />
             <p className="mt-1 font-semibold text-left text-black">
               {bottomData[bottomData.length - 2 + index]?.Result
                 ? bottomData[bottomData.length - 2 + index]?.Result.charAt(
                     0
                   ).toUpperCase() +
                   bottomData[bottomData.length - 2 + index]?.Result.slice(
                     1
                   ).toLowerCase()
                 : "Unknown"}
             </p>
...
export default EggQualityDetection;